...
function renderPayrollDetail(){
  ...
  const actions=[];
  if(editable){actions.push(`<button class="secondary-btn" type="button" onclick="window.recalculatePayroll(${p.id})">คำนวณใหม่</button>`,`<button class="secondary-btn" type="button" onclick="window.openPayrollAdjustment(${p.id})">+ เพิ่ม/หัก/ภาษี</button>`);if(p.status==='draft')actions.push(`<button class="secondary-btn" type="button" onclick="window.reviewPayroll(${p.id})">ส่งตรวจ</button>`);actions.push(`<button class="primary-btn" type="button" onclick="window.lockPayroll(${p.id})">Lock รอบ</button>`);} if(p.status==='locked')actions.push(`<button class="primary-btn" type="button" onclick="window.publishPayroll(${p.id})">Publish Payslip</button>`);
