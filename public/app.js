...
function renderPayrollDetail(){
  ...
  const actions=[];
  if(editable){actions.push(`<button class="secondary-btn" type="button" onclick="window.editPayrollPeriod(${p.id})">แก้ไขรอบ</button>`,`<button class="secondary-btn danger" type="button" onclick="window.deletePayrollPeriod(${p.id})">ลบรอบ</button>`,`<button class="secondary-btn" type="button" onclick="window.recalculatePayroll(${p.id})">คำนวณใหม่</button>`,`<button class="secondary-btn" type="button" onclick="window.openPayrollBulkAdjustment(${p.id})">+ Bulk</button>`,`<button class="secondary-btn" type="button" onclick="window.openPayrollAdjustment(${p.id})">+ รายการ</button>`);if(p.status==='draft')actions.push(`<button class="primary-btn" type="button" onclick="window.reviewPayroll(${p.id})">ส่งตรวจ</button>`);if(p.status==='review'&&p.approval_status==='pending')actions.push(`<button class="primary-btn" type="button" onclick="window.approvePayroll(${p.id})">อนุมัติรอบ</button>`);if(p.status==='review'&&(p.approval_status==='approved'||!Number(settings.require_separate_approver||0)))actions.push(`<button class="primary-btn" type="button" onclick="window.lockPayroll(${p.id})">Lock รอบ</button>`);}
