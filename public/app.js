...
const standardDocumentCatalog=Object.entries(documentTypeMeta).map(([code,m])=>({code,name:m[0],catalog_only:true}));
function renderDocumentTypeCards(){const templates=state.documentSystem?.templates||[],root=$('#documentTypeCards');if(!root)return;if(templates.length){root.innerHTML=templates.map(t=>{const meta=documentTypeMeta[t.code]||[t.name,t.description||'เอกสารพนักงาน','DOC'];return `<button type="button" class="document-type-card" data-template-id="${Number(t.id)}" onclick="selectDocumentTemplate(${Number(t.id)})"><span class="doc-symbol">${escapeHtml(meta[2])}</span><span><strong>${escapeHtml(t.name)}</strong><small>${escapeHtml(meta[1])}</small></span></button>`}).join('');return;}root.innerHTML=standardDocumentCatalog.map(t=>{const meta=documentTypeMeta[t.code];return `<button type="button" class="document-type-card unavailable" onclick="toast('Template ยังโหลดไม่สำเร็จ · กดรีเฟรชแล้วลองอีกครั้ง',true)"><span class="doc-symbol">${escapeHtml(meta[2])}</span><span><strong>${escapeHtml(meta[0])}</strong><small>${escapeHtml(meta[1])}</small></span></button>`}).join('')+`<div class="document-template-error"><strong>โหลด Template ไม่สำเร็จ</strong><small>${escapeHtml(state.documentSystem?.template_load_error||'ระบบยังไม่ได้รับ Template จากเซิร์ฟเวอร์')}</small><button type="button" class="secondary-btn" onclick="refreshDocuments()">ลองใหม่</button></div>`;}
window.selectDocumentTemplate=id=>{const t=(state.documentSystem?.templates||[]).find(x=>Number(x.id)===Number(id));if(!t)return;$('#documentTemplate').value=String(id);document.querySelectorAll('.document-type-card').forEach(x=>x.classList.toggle('active',Number(x.dataset.templateId)===Number(id)));const e=state.employees.find(x=>Number(x.id)===Number($('#documentEmployee').value));const employeeName=e?(e.nickname||`${e.first_name||''} ${e.last_name||''}`.trim()):'พนักงาน';const meta=documentTypeMeta[t.code]||[t.name,'ข้อมูลจากแฟ้มพนักงาน','DOC'];const warning=t.code==='WARNING'?'<br><b>แนะนำ:</b> ใบเตือนควรเริ่มจาก HR Case เพื่อผูกเหตุการณ์ หลักฐาน และคำชี้แจง':'';$('#documentDraftPreview').innerHTML=`<p class="kicker">DOCUMENT PREVIEW</p><strong>${escapeHtml(t.name)} · ${escapeHtml(employeeName)}</strong><small><b>ข้อมูลที่ระบบใช้:</b> ${escapeHtml(meta[1])}<br><b>Workflow:</b> ${t.approval_required?'Draft → HR ตรวจ → อนุมัติ → PDF Final':'สร้างเอกสาร → PDF Final'}${t.acknowledgement_required?' → ส่งให้พนักงานรับทราบ/ชี้แจง':''}${warning}</small>`;};
