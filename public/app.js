...
function renderPayrollExceptions(detail){
  const rows=detail?.exceptions||[]; if(!rows.length)return `<section class="payroll-exception-center is-clear"><div><p class="kicker">VALIDATION CENTER</p><h3>พร้อมตรวจรอบเงินเดือน</h3><span>ไม่พบรายการผิดปกติที่ต้องจัดการ</span></div><b>✓</b></section>`;
  const errors=rows.filter(x=>x.severity==='error'),warnings=rows.filter(x=>x.severity!=='error');
  return `<section class="payroll-exception-center"><div class="payroll-exception-head"><div><p class="kicker">VALIDATION CENTER</p><h3>รายการที่ต้องตรวจ ${rows.length}</h3><span>${errors.length?`${errors.length} รายการต้องแก้ก่อน Lock`:''}${errors.length&&warnings.length?' · ':''}${warnings.length?`${warnings.length} รายการควรตรวจ`:''}</span></div><div class="payroll-exception-count"><b>${errors.length}</b><small>BLOCK</small></div></div><div class="payroll-exception-list">${rows.slice(0,8).map(x=>`<button type="button" class="payroll-exception ${x.severity}" onclick="window.openPayrollEmployeeDetail(${Number(x.employee_id)})"><span><strong>${escapeHtml(x.employee_name)}</strong><small>${escapeHtml(x.title)} · ${escapeHtml(x.detail||'')}</small></span><em>${x.severity==='error'?'ต้องแก้':'ตรวจสอบ'}</em></button>`).join('')}${rows.length>8?`<small class="payroll-more-exceptions">และอีก ${rows.length-8} รายการ</small>`:''}</div></section>`;
}
...
function renderPayrollMobileCards(period,items,editable,settings,adjMap){
  return `<div class="payroll-mobile-list">${items.map(item=>{const a=adjMap.get(Number(item.employee_id))||{commission:0,kpi:0,incentive:0,bonus:0,otherEarn:0,otherDeduct:0};const bd=payrollBreakdown(item),search=`${item.nickname||''} ${item.first_name||''} ${item.last_name||''} ${item.employee_code||''} ${item.department_name||''} ${item.position_name||''}`.toLowerCase(),salaryMode=bd.salary_mode==='full'?'เต็มเดือน':'Prorate',payableDays=Number(bd.payable_days??bd.active_calendar_days??0);return `<article class="payroll-mobile-card" data-payroll-search="${escapeAttr(search)}"><div class="payroll-mobile-card-head"><button class="payroll-person payroll-person-button" type="button" onclick="window.openPayrollEmployeeDetail(${Number(item.employee_id)})"><span class="payroll-person-avatar">${escapeHtml((item.nickname||item.first_name||'?').slice(0,1))}</span><span><strong>${escapeHtml(item.nickname||item.first_name)} ${escapeHtml(item.last_name||'')}</strong><small>${escapeHtml(item.employee_code||'')} · ${escapeHtml(item.department_name||'—')}</small></span></button><div class="payroll-mobile-net"><span>รับสุทธิ</span><strong>${money(item.net_pay)}</strong></div></div><div class="payroll-mobile-meta"><span>${escapeHtml(item.position_name||'ยังไม่ระบุตำแหน่ง')}</span><span>เริ่มงาน ${item.start_date?formatDate(item.start_date):'—'}</span><span>${salaryMode} · ${payableDays||0} วัน</span></div><div class="payroll-mobile-sections"><section class="payroll-mobile-section"><div class="payroll-mobile-section-title">ข้อมูลเงินเดือน</div><div class="payroll-mobile-pairs"><div><span>ฐานเงินเดือน</span><strong>${money(item.base_salary)}</strong></div><div><span>เงินเดือนรอบนี้</span><strong>${money(item.prorated_salary)}</strong></div><div><span>วันคิดเงิน</span><strong>${payableDays||'—'}</strong><small>ตัวหาร ${Number(settings.daily_rate_divisor||30)}</small></div><div><span>รูปแบบ</span><strong>${salaryMode}</strong></div></div></section><section class="payroll-mobile-section"><div class="payroll-mobile-section-title">รายได้แปรผัน</div><div class="payroll-mobile-edit-grid"><label><span>Commission</span>${payrollGridInput(period.id,item.employee_id,'commission',a.commissionGrid,editable)}</label><label><span>KPI</span>${payrollGridInput(period.id,item.employee_id,'kpi',a.kpiGrid,editable)}</label><label><span>Incentive</span>${payrollGridInput(period.id,item.employee_id,'incentive',a.incentiveGrid,editable)}</label><label><span>Bonus</span>${payrollGridInput(period.id,item.employee_id,'bonus',a.bonusGrid,editable)}</label><label class="full"><span>อื่น ๆ</span>${payrollGridInput(period.id,item.employee_id,'other',a.otherGrid,editable)}</label></div></section><section class="payroll-mobile-section"><div class="payroll-mobile-section-title">รายการหัก</div><div class="payroll-mobile-pairs"><div><span>ขาด/สาย</span><strong>${Number(item.attendance_deduction||0)?money(item.attendance_deduction):'—'}</strong><small>${Number(item.absent_days||0)} วัน · ${Number(item.late_minutes||0)} นาที</small></div><div><span>ปกส.</span><strong>${money(item.social_security)}</strong></div><div><span>ภาษี</span><strong>${money(item.withholding_tax)}</strong></div><div><span>หักอื่น</span>${payrollGridInput(period.id,item.employee_id,'other_deduction',a.otherDeductGrid,editable)}</div></div></section><section class="payroll-mobile-section"><div class="payroll-mobile-section-title">สรุปรอบนี้</div><div class="payroll-mobile-pairs payroll-mobile-pairs-highlight"><div><span>Gross</span><strong>${money(item.gross_income)}</strong></div><div><span>Net Pay</span><strong class="net">${money(item.net_pay)}</strong></div><div><span>Employer Cost</span><strong>${money(item.employer_cost||item.gross_income)}</strong><small>บริษัท ปกส. ${money(item.employer_social_security||0)}</small></div><div><span>เทียบรอบก่อน</span><strong>${payrollVarianceBadge(item)}</strong><small>${Number(item.prior_net_pay||0)?money(item.prior_net_pay):'ไม่มีรอบก่อน'}</small></div></div></section><section class="payroll-mobile-section"><div class="payroll-mobile-section-title">บัญชีรับเงิน</div><div class="payroll-mobile-pairs"><div><span>ธนาคาร</span><strong>${escapeHtml(item.bank_name||'—')}</strong></div><div><span>เลขบัญชี</span><strong>${item.bank_account_no?escapeHtml(String(item.bank_account_no)):'—'}</strong></div><div class="full"><span>ชื่อบัญชี</span><strong>${escapeHtml(item.bank_account_name||'—')}</strong>${item.payroll_note?`<small>${escapeHtml(item.payroll_note)}</small>`:''}</div></div></section></div><div class="payroll-mobile-actions">${editable?`<button class="secondary-btn" type="button" onclick="window.openPayrollAdjustment(${period.id},${item.employee_id})">ปรับรายการ</button>`:''}<button class="primary-btn ghost" type="button" onclick="window.openPayrollEmployeeDetail(${item.employee_id})">ดูรายละเอียด</button></div></article>`;}).join('')}</div>`;
}
function renderPayrollDetail(){
  $('#payrollDetail')?.classList.remove('payroll-detail-empty');
  const detail=state.payrollDetail; if(!detail?.period)return; const p=detail.period; const editable=['draft','review'].includes(p.status); const documents=detail.documents||[]; const adjMap=payrollAdjustmentMaps(detail); const items=detail.items||[]; const settings=detail.settings||state.payroll?.settings||{};
  const actions=[];
  if(editable){actions.push(`<button class="secondary-btn" type="button" onclick="window.recalculatePayroll(${p.id})">คำนวณใหม่</button>`,`<button class="secondary-btn" type="button" onclick="window.openPayrollBulkAdjustment(${p.id})">+ Bulk</button>`,`<button class="secondary-btn" type="button" onclick="window.openPayrollAdjustment(${p.id})">+ รายการ</button>`);if(p.status==='draft')actions.push(`<button class="primary-btn" type="button" onclick="window.reviewPayroll(${p.id})">ส่งตรวจ</button>`);if(p.status==='review'&&p.approval_status==='pending')actions.push(`<button class="primary-btn" type="button" onclick="window.approvePayroll(${p.id})">อนุมัติรอบ</button>`);if(p.status==='review'&&(p.approval_status==='approved'||!Number(settings.require_separate_approver||0)))actions.push(`<button class="primary-btn" type="button" onclick="window.lockPayroll(${p.id})">Lock รอบ</button>`);}
  if(p.status==='locked')actions.push(`<button class="secondary-btn" type="button" onclick="window.generatePayrollPayslips(${p.id})">สร้างสลิป</button>`,`<button class="secondary-btn" type="button" onclick="window.exportPayroll(${p.id},'bank')">Bank CSV</button>`,`<button class="secondary-btn" type="button" onclick="window.exportPayroll(${p.id},'accounting')">Accounting CSV</button>`,`<button class="secondary-btn danger" type="button" onclick="window.unlockPayroll(${p.id})">ปลด Lock</button>`,`<button class="primary-btn" type="button" onclick="window.publishPayroll(${p.id})">ส่ง Payslip</button>`);
  if(p.status==='published')actions.push(`<button class="secondary-btn" type="button" onclick="window.exportPayroll(${p.id},'bank')">Bank CSV</button>`,`<button class="secondary-btn" type="button" onclick="window.exportPayroll(${p.id},'accounting')">Accounting CSV</button>`);
  const employerCost=items.reduce((a,x)=>a+Number(x.employer_cost||x.gross_income||0),0),taxTotal=items.reduce((a,x)=>a+Number(x.withholding_tax||0),0),ssoTotal=items.reduce((a,x)=>a+Number(x.social_security||0),0);
  $('#payrollDetail').innerHTML=`<div class="payroll-detail-head payroll-detail-head-mobile"><div><p class="kicker">PAYROLL CONTROL CENTER · ${escapeHtml(p.period_key)}</p><h2>เงินเดือน ${escapeHtml(p.period_key)}</h2><div class="payroll-period-range">${formatDate(p.period_start)} → ${formatDate(p.period_end)} <span>${payrollRangeDays(p.period_start,p.period_end)} วัน</span></div><div class="payroll-status-line"><span class="badge ${payrollStatusClass(p.status)}">${payrollStatusLabel(p.status)}</span><span class="payroll-approval-pill ${p.approval_status||'not_required'}">${escapeHtml(payrollApprovalLabel(p))}</span><small>จ่าย ${formatDate(p.pay_date)}${p.locked_at?` · Lock ${formatDateTime(p.locked_at)}`:''}</small></div></div><div class="page-actions payroll-page-actions-mobile">${actions.join('')}</div></div>
    <div class="payroll-detail-totals payroll-detail-totals-v2"><div><span>Gross Payroll</span><strong>${money(p.gross_total)}</strong></div><div><span>ภาษีหัก ณ ที่จ่าย</span><strong>${money(taxTotal)}</strong></div><div><span>ประกันสังคมพนักงาน</span><strong>${money(ssoTotal)}</strong></div><div class="net"><span>ยอดโอนสุทธิ</span><strong>${money(p.net_total)}</strong></div><div><span>Employer Cost</span><strong>${money(employerCost)}</strong></div></div>
    ${renderPayrollExceptions(detail)}
    <div class="payroll-grid-toolbar payroll-grid-toolbar-mobile"><div><input id="payrollGridSearch" type="search" placeholder="ค้นหาชื่อ / รหัส / แผนก" oninput="window.filterPayrollGrid(this.value)" /><span>${items.length} คน · รอบ ${formatDate(p.period_start)} → ${formatDate(p.period_end)} · Attendance/Leave จะดึงตามช่วงนี้</span></div><button class="secondary-btn" type="button" onclick="window.openPayrollComponentsModal()">จัดการรายการเงิน</button></div>
    <div class="payroll-sheet-hint payroll-sheet-hint-mobile"><span>มือถือจะแสดงเป็นการ์ดแยกคน เพื่อให้อ่านง่ายและแก้ไขรายการได้ตรงจุด</span><span>ถ้าใช้คอม ระบบยังมีตารางเต็มพร้อม Freeze หัวตารางและชื่อพนักงาน</span></div>
    ${renderPayrollMobileCards(p,items,editable,settings,adjMap)}
    <div class="payroll-sheet-wrap payroll-control-grid payroll-desktop-grid"><table class="payroll-sheet payroll-sheet-v2 payroll-sheet-company"><thead><tr class="payroll-group-head"><th class="sticky-col employee-col" rowspan="2">พนักงาน</th><th colspan="6" class="group-fixed">ข้อมูลพนักงาน / เงินเดือน</th><th colspan="5" class="group-variable">รายได้แปรผัน</th><th colspan="4" class="group-deduct">รายการหัก</th><th colspan="4" class="group-result">ผลลัพธ์</th><th colspan="3" class="group-bank">บัญชีรับเงิน</th><th class="sticky-action" rowspan="2">จัดการ</th></tr><tr><th>แผนก / ตำแหน่ง</th><th>วันที่เริ่มงาน</th><th>ฐานเงินเดือน</th><th>รูปแบบ</th><th>วันคิดเงิน</th><th>เงินเดือนรอบนี้</th><th>Commission</th><th>KPI</th><th>Incentive</th><th>Bonus</th><th>อื่น ๆ</th><th>ขาด/สาย</th><th>ปกส.</th><th>ภาษี</th><th>หักอื่น</th><th>Gross</th><th>Net Pay</th><th>Δ รอบก่อน</th><th>Employer Cost</th><th>ธนาคาร</th><th>เลขบัญชี</th><th>ชื่อบัญชี</th></tr></thead><tbody>${items.map(item=>{const a=adjMap.get(Number(item.employee_id))||{commission:0,kpi:0,incentive:0,bonus:0,otherEarn:0,otherDeduct:0};const bd=payrollBreakdown(item),search=`${item.nickname||''} ${item.first_name||''} ${item.last_name||''} ${item.employee_code||''} ${item.department_name||''}`.toLowerCase(),salaryMode=bd.salary_mode==='full'?'เต็มเดือน':'Prorate',payableDays=Number(bd.payable_days??bd.active_calendar_days??0);return `<tr data-payroll-search="${escapeAttr(search)}"><td class="sticky-col employee-col"><button class="payroll-person payroll-person-button" type="button" onclick="window.openPayrollEmployeeDetail(${Number(item.employee_id)})"><span class="payroll-person-avatar">${escapeHtml((item.nickname||item.first_name||'?').slice(0,1))}</span><span><strong>${escapeHtml(item.nickname||item.first_name)} ${escapeHtml(item.last_name||'')}</strong><small>${escapeHtml(item.employee_code||'')}</small></span></button></td><td><strong>${escapeHtml(item.department_name||'—')}</strong><small>${escapeHtml(item.position_name||'—')}</small></td><td>${item.start_date?formatDate(item.start_date):'—'}</td><td>${money(item.base_salary)}</td><td><span class="payroll-salary-mode ${bd.salary_mode==='full'?'full':'partial'}">${salaryMode}</span></td><td><strong>${payableDays||'—'}</strong><small>ตัวหาร ${Number(settings.daily_rate_divisor||30)}</small></td><td>${money(item.prorated_salary)}</td><td class="editable-pay-cell">${payrollGridInput(p.id,item.employee_id,'commission',a.commissionGrid,editable)}${Number(a.commission||0)?`<small>รวม ${money(a.commission)}</small>`:''}</td><td class="editable-pay-cell">${payrollGridInput(p.id,item.employee_id,'kpi',a.kpiGrid,editable)}${Number(a.kpi||0)?`<small>รวม ${money(a.kpi)}</small>`:''}</td><td class="editable-pay-cell">${payrollGridInput(p.id,item.employee_id,'incentive',a.incentiveGrid,editable)}${Number(a.incentive||0)?`<small>รวม ${money(a.incentive)}</small>`:''}</td><td class="editable-pay-cell">${payrollGridInput(p.id,item.employee_id,'bonus',a.bonusGrid,editable)}${Number(a.bonus||0)?`<small>รวม ${money(a.bonus)}</small>`:''}</td><td class="editable-pay-cell">${payrollGridInput(p.id,item.employee_id,'other',a.otherGrid,editable)}${Number(a.otherEarn||0)?`<small>รวม ${money(a.otherEarn)}</small>`:''}</td><td>${Number(item.attendance_deduction||0)?money(item.attendance_deduction):'—'}<small>${Number(item.absent_days||0)} วัน · ${Number(item.late_minutes||0)} นาที</small></td><td>${money(item.social_security)}</td><td>${money(item.withholding_tax)}</td><td class="editable-pay-cell">${payrollGridInput(p.id,item.employee_id,'other_deduction',a.otherDeductGrid,editable)}${Number(a.otherDeduct||0)?`<small>รวม ${money(a.otherDeduct)}</small>`:''}</td><td class="gross-col"><strong>${money(item.gross_income)}</strong></td><td class="net-col"><strong>${money(item.net_pay)}</strong></td><td>${payrollVarianceBadge(item)}<small>${Number(item.prior_net_pay||0)?money(item.prior_net_pay):'ไม่มีรอบก่อน'}</small></td><td><strong>${money(item.employer_cost||item.gross_income)}</strong><small>บริษัท ปกส. ${money(item.employer_social_security||0)}</small></td><td><strong>${escapeHtml(item.bank_name||'—')}</strong></td><td>${item.bank_account_no?escapeHtml(String(item.bank_account_no)):'—'}</td><td>${escapeHtml(item.bank_account_name||'—')}${item.payroll_note?`<small>${escapeHtml(item.payroll_note)}</small>`:''}</td><td class="sticky-action">${editable?`<button class="payroll-adjust-btn" onclick="window.openPayrollAdjustment(${p.id},${item.employee_id})">ปรับ</button>`:''}<button class="text-btn" onclick="window.openPayrollEmployeeDetail(${item.employee_id})">ดู</button></td></tr>`;}).join('')}</tbody><tfoot><tr><td class="sticky-col employee-col"><strong>รวมทั้งรอบ</strong></td><td colspan="15"></td><td class="gross-col"><strong>${money(p.gross_total)}</strong></td><td class="net-col"><strong>${money(p.net_total)}</strong></td><td></td><td><strong>${money(employerCost)}</strong></td><td colspan="3"></td><td class="sticky-action"></td></tr></tfoot></table></div>
    ${renderPayrollAdjustments(detail,editable)}${renderPayrollTimeline(detail)}
    <div class="payroll-footnote"><span>ภาษีเป็นประมาณการจาก Tax Profile / YTD และ Rule version ของรอบ · ตรวจสอบก่อน Lock โดยเฉพาะพนักงานที่มีรายได้จากแหล่งอื่นหรือค่าลดหย่อนเพิ่มเติม</span><span>${documents.length?`Payslip ${documents.length}/${items.length}`:''}</span></div>`;
...
function openPayrollPeriodModal(){const now=new Date();const key=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;$('#payrollPeriodKey').value=key;syncPayrollPeriodRangeFromMonth();$('#payrollPeriodKey').onchange=syncPayrollPeriodRangeFromMonth;$('#payrollPeriodStart').onchange=updatePayrollPeriodRangePreview;$('#payrollPeriodEnd').onchange=updatePayrollPeriodRangePreview;$('#payrollPeriodModal').showModal();}
...
async function createPayrollPeriod(){const button=$('#payrollPeriodCreateBtn');button.disabled=true;button.textContent='กำลังคำนวณ…';try{const start=$('#payrollPeriodStart').value,end=$('#payrollPeriodEnd').value;if(!start||!end)throw new Error('กรุณาระบุวันที่เริ่มและสิ้นสุดรอบเงินเดือน');if(start>end)throw new Error('วันที่เริ่มรอบต้องไม่เกินวันที่สิ้นสุดรอบ');const result=await api('/api/payroll/periods',{method:'POST',body:JSON.stringify({period_key:$('#payrollPeriodKey').value,period_start:start,period_end:end,pay_date:$('#payrollPayDate').value})});$('#payrollPeriodModal').close();state.activePayrollPeriodId=result.id;await refreshPayroll();await loadPayrollPeriod(result.id);toast(`สร้าง Payroll Preview ${formatDate(start)} → ${formatDate(end)} แล้ว`);}catch(e){toast(e.message,true)}finally{button.disabled=false;button.textContent='สร้างและคำนวณ Preview';}}
...
window.openPayrollEmployeeDetail=employeeId=>{const item=(state.payrollDetail?.items||[]).find(x=>Number(x.employee_id)===Number(employeeId));if(!item)return;const bd=payrollBreakdown(item),rec=bd.recurring_components||[];$('#payrollEmployeeDetailTitle').textContent=`${item.nickname||item.first_name||''} ${item.last_name||''}`.trim();$('#payrollEmployeeDetailBody').innerHTML=`<div class="payroll-employee-kpis"><div><span>Gross</span><strong>${money(item.gross_income)}</strong></div><div><span>Net</span><strong>${money(item.net_pay)}</strong></div><div><span>YTD Gross</span><strong>${money(item.ytd_gross)}</strong></div><div><span>YTD Tax</span><strong>${money(item.ytd_tax)}</strong></div></div><div class="payroll-employee-detail-grid"><section><h4>รายได้ประจำ</h4><p>ฐานเงินเดือน <b>${money(item.base_salary)}</b></p>${rec.map(x=>`<p>${escapeHtml(x.name)} <b>${money(x.amount)}</b></p>`).join('')||'<p class="muted">ไม่มีรายการเพิ่มประจำ</p>'}</section><section><h4>ภาษี / ประกันสังคม</h4><p>ภาษีรอบนี้ <b>${money(item.withholding_tax)}</b></p><p>ประกันสังคมพนักงาน <b>${money(item.social_security)}</b></p><p>ประกันสังคมบริษัท <b>${money(item.employer_social_security)}</b></p><p>Tax Rule <b>${escapeHtml(bd.tax_rule||'—')}</b></p></section><section><h4>Attendance</h4><p>ขาดงาน <b>${Number(item.absent_days||0)} วัน</b></p><p>มาสาย <b>${Number(item.late_minutes||0)} นาที</b></p><p>หัก Attendance <b>${money(item.attendance_deduction)}</b></p></section><section><h4>การจ่ายเงิน</h4><p>ธนาคาร <b>${escapeHtml(item.bank_name||'ยังไม่ระบุ')}</b></p><p>เลขบัญชี <b>${escapeHtml(item.bank_account_no||'ยังไม่ระบุ')}</b></p><p>เทียบเดือนก่อน ${payrollVarianceBadge(item)}</p></section></div>`;$('#payrollEmployeeDetailModal').showModal();};
