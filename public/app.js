...
function payrollManualOverrideInput(periodId,employeeId,category,value,editable,{isOverride=false,derivedFromDays=false,autoValue=null}={}){
  const isDays=category==='payable_days';
  if(!editable)return isDays?`<strong>${Number(value||0)}</strong>`:money(value);
  const numeric=Number(value??0),step=isDays?'0.5':'0.01',hint=derivedFromDays?'ตามวันแก้เอง':'Auto';
  return `<div class="payroll-manual-input-wrap ${isOverride?'is-overridden':''} ${derivedFromDays?'is-derived':''}"><input class="payroll-grid-input payroll-manual-input ${isDays?'days':'salary'}" inputmode="decimal" type="number" min="0" step="${step}" value="${Number.isFinite(numeric)?numeric:0}" onchange="window.savePayrollManualCell(${Number(periodId)},${Number(employeeId)},'${category}',this)" />${isOverride?`<button type="button" class="payroll-auto-reset" title="กลับไปใช้ค่าที่ระบบคำนวณ" onclick="window.resetPayrollManualCell(${Number(periodId)},${Number(employeeId)},'${category}',this)">Auto</button>`:`<small class="payroll-auto-hint">${escapeHtml(hint)}${autoValue!=null&&!derivedFromDays?` ${isDays?Number(autoValue):money(autoValue)}`:''}</small>`}</div>`;
}
...
function renderPayrollTools(p,editable){
  ...
  if(p.status==='review')tools.push(`<button type="button" onclick="window.createPayrollReviewExcel(${p.id})">สร้าง Excel ล่าสุด</button>`);
  if(p.status==='locked')tools.push(`<button type="button" onclick="window.generatePayrollPayslips(${p.id})">สร้างสลิปก่อนส่ง</button>`,`<button type="button" onclick="window.createPayrollReviewExcel(${p.id})">สร้าง Excel ล่าสุด</button>`,`<button type="button" onclick="window.exportPayroll(${p.id},'bank')">Export ธนาคาร CSV</button>`,`<button type="button" onclick="window.exportPayroll(${p.id},'accounting')">Export บัญชี</button>`,`<button class="danger" type="button" onclick="window.unlockPayroll(${p.id})">ปลด Lock</button>`);
  if(p.status==='published')tools.push(`<button type="button" onclick="window.createPayrollReviewExcel(${p.id})">สร้าง Excel ล่าสุด</button>`,`<button type="button" onclick="window.exportPayroll(${p.id},'bank')">Export ธนาคาร CSV</button>`,`<button type="button" onclick="window.exportPayroll(${p.id},'accounting')">Export บัญชี</button>`);
  if(!tools.length)return '';
  return `<details class="payroll-more-actions"><summary>เครื่องมือเพิ่มเติม</summary><div>${tools.join('')}</div></details>`;
}
function renderPayrollTimeline(detail){const rows=detail?.timeline||[];if(!rows.length)return '';const labels={period_created:'สร้างรอบ',submitted_for_review:'ส่งตรวจ',approved:'อนุมัติ',locked:'Lock รอบ',unlocked:'ปลด Lock',published:'Publish',grid_cell_updated:'แก้ตาราง',salary_override_updated:'แก้วัน/เงินเดือนรอบ',bulk_adjustment:'เพิ่มรายการหลายคน',bank_exported:'Export ธนาคาร',accounting_exported:'Export บัญชี',review_excel_created:'สร้าง Excel รอบตรวจ'};return `<details class="payroll-timeline"><summary>ประวัติรอบเงินเดือน <span>${rows.length}</span></summary><div>${rows.slice(0,30).map(x=>`<article><b>${escapeHtml(labels[x.event_type]||x.event_type)}</b><span>${escapeHtml(x.actor_name||x.actor_email||'ระบบ')}</span><time>${formatDateTime(x.created_at)}</time></article>`).join('')}</div></details>`;}

function renderPayrollExports(detail){
  const rows=detail?.exports||[],p=detail?.period||{};
  if(!rows.length)return '';
  const latest=rows[0],stale=Boolean(latest.source_updated_at&&p.updated_at&&String(latest.source_updated_at)!==String(p.updated_at));
  const history=rows.slice(1,6);
  return `<section class="payroll-export-panel ${stale?'is-stale':''}"><div class="payroll-export-head"><div><span class="payroll-export-badge">XLSX</span><div><strong>ไฟล์โอนเงินของรอบนี้</strong><small>${stale?'มีการแก้ตัวเลขหลังสร้างไฟล์ · ควรสร้างใหม่ก่อนโอน':'บันทึก Snapshot ไว้ในระบบแล้ว · ดาวน์โหลดซ้ำได้ทุกเมื่อ'}</small></div></div><button class="secondary-btn payroll-export-new" type="button" onclick="window.createPayrollReviewExcel(${Number(p.id)})">${stale?'สร้างไฟล์ล่าสุด':'สร้างใหม่'}</button></div><div class="payroll-export-latest"><div class="payroll-export-file"><strong>${escapeHtml(latest.file_name||'Payroll.xlsx')}</strong><span>${Number(latest.row_count||0)} คน · ยอดโอน ${money(latest.net_total||0)} · ${formatDateTime(latest.created_at)}</span>${Number(latest.missing_bank_count||0)>0?`<small class="warning">มี ${Number(latest.missing_bank_count)} คนข้อมูลบัญชีไม่ครบ</small>`:'<small class="ready">ข้อมูลบัญชีอยู่ในไฟล์พร้อมสำหรับคัดลอก</small>'}</div><button class="primary-btn ghost" type="button" onclick="window.downloadPayrollSavedExport('/api/payroll/exports/${Number(latest.id)}/download.xlsx','${escapeAttr(latest.file_name||'Payroll.xlsx')}')">ดาวน์โหลด Excel</button></div>${history.length?`<details class="payroll-export-history"><summary>ไฟล์ก่อนหน้า <span>${history.length}</span></summary><div>${history.map(x=>`<article><span><strong>${escapeHtml(x.file_name||'Payroll.xlsx')}</strong><small>${formatDateTime(x.created_at)} · ${Number(x.row_count||0)} คน</small></span><button type="button" onclick="window.downloadPayrollSavedExport('/api/payroll/exports/${Number(x.id)}/download.xlsx','${escapeAttr(x.file_name||'Payroll.xlsx')}')">ดาวน์โหลด</button></article>`).join('')}</div></details>`:''}</section>`;
}
function renderPayrollMobileCards(period,items,editable,settings,adjMap){
  return `<div class="payroll-mobile-list">${items.map(item=>{const a=adjMap.get(Number(item.employee_id))||{commission:0,kpi:0,incentive:0,bonus:0,otherEarn:0,otherDeduct:0};const bd=payrollBreakdown(item),search=`${item.nickname||''} ${item.first_name||''} ${item.last_name||''} ${item.employee_code||''} ${item.department_name||''} ${item.position_name||''}`.toLowerCase();const deductions=Number(item.gross_income||0)-Number(item.net_pay||0),payableDays=Number(bd.payable_days??bd.active_calendar_days??0),autoPayableDays=Number(bd.auto_payable_days??payableDays),autoProrated=Number(bd.auto_prorated_salary??item.prorated_salary??0),dayOverride=item.payable_days_override!==null&&item.payable_days_override!==undefined,salaryOverride=item.prorated_salary_override!==null&&item.prorated_salary_override!==undefined;return `<article class="payroll-mobile-card payroll-mobile-card-compact" data-payroll-search="${escapeAttr(search)}"><div class="payroll-mobile-card-head"><button class="payroll-person payroll-person-button" type="button" onclick="window.openPayrollEmployeeDetail(${Number(item.employee_id)})"><span class="payroll-person-avatar">${escapeHtml((item.nickname||item.first_name||'?').slice(0,1))}</span><span><strong>${escapeHtml(item.nickname||item.first_name)} ${escapeHtml(item.last_name||'')}</strong><small>${escapeHtml(item.employee_code||'')} · ${escapeHtml(item.department_name||'—')}</small></span></button><div class="payroll-mobile-net"><span>รับสุทธิ</span><strong>${money(item.net_pay)}</strong></div></div><div class="payroll-mobile-three"><div><span>รายได้รวม</span><strong>${money(item.gross_income)}</strong></div><div><span>หักทั้งหมด</span><strong>${money(Math.max(0,deductions))}</strong></div><div><span>สถานะ</span><strong>${String(item.bank_account_no||'').trim()?'พร้อม':'ขาดบัญชี'}</strong></div></div>${editable?`<div class="payroll-mobile-quick-edit payroll-mobile-quick-edit-v2"><label><span>วันคิดเงิน</span>${payrollManualOverrideInput(period.id,item.employee_id,'payable_days',payableDays,editable,{isOverride:dayOverride,autoValue:autoPayableDays})}</label><label><span>เงินเดือนรอบนี้</span>${payrollManualOverrideInput(period.id,item.employee_id,'prorated_salary',item.prorated_salary,editable,{isOverride:salaryOverride,derivedFromDays:dayOverride&&!salaryOverride,autoValue:autoProrated})}</label><label><span>Commission</span>${payrollGridInput(period.id,item.employee_id,'commission',a.commissionGrid,editable)}</label><label><span>KPI</span>${payrollGridInput(period.id,item.employee_id,'kpi',a.kpiGrid,editable)}</label><label><span>Incentive</span>${payrollGridInput(period.id,item.employee_id,'incentive',a.incentiveGrid,editable)}</label><label><span>Bonus</span>${payrollGridInput(period.id,item.employee_id,'bonus',a.bonusGrid,editable)}</label></div>`:''}<details class="payroll-mobile-more"><summary>ดูรายละเอียดเงินเดือน</summary><div class="payroll-mobile-more-body"><div class="payroll-mobile-pairs"><div><span>ฐานเงินเดือน</span><strong>${money(item.base_salary)}</strong></div><div><span>เงินเดือนรอบนี้</span><strong>${money(item.prorated_salary)}</strong></div><div><span>ขาด/สาย</span><strong>${money(item.attendance_deduction||0)}</strong><small>${Number(item.absent_days||0)} วัน · ${Number(item.late_minutes||0)} นาที</small></div><div><span>ประกันสังคม</span><strong>${money(item.social_security)}</strong></div><div><span>ภาษี</span><strong>${money(item.withholding_tax)}</strong></div><div><span>ต้นทุนบริษัท</span><strong>${money(item.employer_cost||item.gross_income)}</strong></div><div><span>ธนาคาร</span><strong>${escapeHtml(item.bank_name||'—')}</strong></div><div><span>เลขบัญชี</span><strong>${item.bank_account_no?escapeHtml(String(item.bank_account_no)):'—'}</strong></div></div></div></details><div class="payroll-mobile-actions">${editable?`<button class="secondary-btn" type="button" onclick="window.openPayrollAdjustment(${period.id},${item.employee_id})">เพิ่ม/หักเงิน</button>`:''}<button class="primary-btn ghost" type="button" onclick="window.openPayrollEmployeeDetail(${item.employee_id})">ดูที่มาของตัวเลข</button></div></article>`;}).join('')}</div>`;
}
