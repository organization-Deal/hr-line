...
function renderPayroll(){
  ...
  if(periods.length){
    root.innerHTML=[
      ['รอบล่าสุด',latest?.period_key||'—','calendar','รอบเงินเดือนที่กำลังใช้งาน'],
...
function renderPayrollSetupList(){
  ...
  const rows=profiles.map(e=>{const status=payrollSetupStatus(e);const search=`${e.nickname||''} ${e.first_name||''} ${e.last_name||''} ${e.employee_code||''} ${e.department_name||''} ${e.position_name||''}`.toLowerCase();const tax=e.tax_enabled==null||Number(e.tax_enabled)===1;const sso=e.social_security_enabled==null||Number(e.social_security_enabled)===1;return `<tr data-payroll-setup-row data-payroll-setup-search="${escapeAttr(search)}" data-payroll-setup-status="${status.key}"><td class="payroll-setup-employee"><button type="button" onclick="window.openPayrollProfile(${Number(e.id)})"><span class="payroll-person-avatar">${escapeHtml((e.nickname||e.first_name||'?').slice(0,1))}</span><span><strong>${escapeHtml(e.nickname||e.first_name)} ${escapeHtml(e.last_name||'')}</strong><small>${escapeHtml(e.employee_code||'—')}</small></span></button></td><td><strong>${escapeHtml(e.department_name||'ยังไม่ระบุ')}</strong><small>${escapeHtml(e.position_name||'ยังไม่ระบุตำแหน่ง')}</small></td><td class="payroll-setup-money">${Number(e.base_salary||0)>0?`<strong>${money(e.base_salary)}</strong><small>มีผล ${e.effective_from?formatDate(e.effective_from):'ตามข้อมูลล่าสุด'}</small>`:'<span class="payroll-missing-value">ยังไม่ตั้ง</span>'}</td><td><div class="payroll-rule-badges"><span class="${tax?'on':'off'}">ภาษี ${tax?'เปิด':'ปิด'}</span><span class="${sso?'on':'off'}">SSO ${sso?'เปิด':'ปิด'}</span></div></td><td>${e.bank_account_no?`<strong>${escapeHtml(e.bank_name||'บัญชีรับเงิน')}</strong><small>•••• ${escapeHtml(String(e.bank_account_no).slice(-4))}</small>`:'<span class="payroll-missing-value">ยังไม่มีบัญชี</span>'}</td><td><span class="payroll-setup-status ${status.className}">${escapeHtml(status.label)}</span></td><td class="payroll-setup-action"><button type="button" class="secondary-btn" onclick="window.openPayrollProfile(${Number(e.id)})">${status.key==='ready'?'แก้ไข':'ตั้งค่า'}</button></td></tr>`;}).join('');
  detail.innerHTML=`<div class="payroll-setup-head"><div><p class="kicker">EMPLOYEE PAYROLL SETUP</p><h2>ตั้งข้อมูลเงินเดือนพนักงาน</h2><p>ตั้งฐานเงินเดือน ภาษี ประกันสังคม และบัญชีรับเงินให้ครบก่อนสร้างรอบแรก</p></div><div class="payroll-setup-head-status"><strong>${Number(ready.salary_ready||0)}/${Number(ready.employees||0)}</strong><span>ฐานเงินเดือนพร้อม</span></div></div><div class="payroll-setup-toolbar"><div class="payroll-setup-search"><input id="payrollSetupSearch" type="search" placeholder="ค้นหาชื่อ / รหัส / แผนก" oninput="window.filterPayrollSetup()"/><select id="payrollSetupStatusFilter" onchange="window.filterPayrollSetup()"><option value="all">ทุกสถานะ</option><option value="missing">ขาดหลายรายการ</option><option value="salary">ขาดฐานเงินเดือน</option><option value="bank">ขาดบัญชีรับเงิน</option><option value="ready">พร้อมแล้ว</option></select></div><span id="payrollSetupCount">${profiles.length} คน${missing?` · ต้องตั้งค่าอีก ${missing} คน`:''}</span></div>${profiles.length?`<div class="payroll-setup-table-wrap"><table class="payroll-setup-table"><thead><tr><th>พนักงาน</th><th>แผนก / ตำแหน่ง</th><th>ฐานเงินเดือน</th><th>ภาษี / ประกันสังคม</th><th>บัญชีรับเงิน</th><th>สถานะ</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>`:emptyState('ยังไม่มีพนักงาน','เพิ่มพนักงานก่อนตั้ง Payroll')}`;
}
...
function payrollAdjustmentMaps(detail){
  ...
  for(const a of detail?.adjustments||[]){
    const id=Number(a.employee_id); const row=map.get(id)||{commission:0,kpi:0,incentive:0,bonus:0,otherEarn:0,otherDeduct:0,taxAdjust:0,commissionGrid:0,incentiveGrid:0,bonusGrid:0,otherGrid:0,otherDeductGrid:0,items:[]};
    const amt=Number(a.amount||0), cat=String(a.category||'other'), source=String(a.source_key||''); row.items.push(a);
    if(source==='grid:commission')row.commissionGrid=amt; if(source==='grid:incentive')row.incentiveGrid=amt; if(source==='grid:bonus')row.bonusGrid=amt; if(source==='grid:other')row.otherGrid=amt; if(source==='grid:other_deduction')row.otherDeductGrid=amt;
...
function renderPayrollDetail(){
  ...
  $('#payrollDetail').innerHTML=`<div class="payroll-detail-head"><div><p class="kicker">PAYROLL CONTROL CENTER · ${escapeHtml(p.period_key)}</p><h2>เงินเดือน ${escapeHtml(p.period_key)}</h2><div class="payroll-status-line"><span class="badge ${payrollStatusClass(p.status)}">${payrollStatusLabel(p.status)}</span><span class="payroll-approval-pill ${p.approval_status||'not_required'}">${escapeHtml(payrollApprovalLabel(p))}</span><small>จ่าย ${formatDate(p.pay_date)}${p.locked_at?` · Lock ${formatDateTime(p.locked_at)}`:''}</small></div></div><div class="page-actions">${actions.join('')}</div></div>
    ...
    <div class="payroll-grid-toolbar"><div><input id="payrollGridSearch" type="search" placeholder="ค้นหาชื่อ / รหัส / แผนก" oninput="window.filterPayrollGrid(this.value)" /><span>${items.length} คน · Attendance cutoff วันที่ ${Number(settings.attendance_cutoff_day||25)} · Commission cutoff วันที่ ${Number(settings.commission_cutoff_day||25)}</span></div><button class="secondary-btn" type="button" onclick="window.openPayrollComponentsModal()">จัดการรายการเงิน</button></div>
    <div class="payroll-sheet-hint"><span>ช่องพื้นขาวที่มีกรอบสามารถพิมพ์ตัวเลขแล้วกด Enter/แตะนอกช่องเพื่อคำนวณใหม่ได้</span><span>ชื่อพนักงานและหัวตารางจะ Freeze ขณะเลื่อน</span></div>
    <div class="payroll-sheet-wrap payroll-control-grid"><table class="payroll-sheet payroll-sheet-v2"><thead><tr class="payroll-group-head"><th class="sticky-col employee-col" rowspan="2">พนักงาน</th><th colspan="4" class="group-fixed">รายได้ประจำ</th><th colspan="4" class="group-variable">รายได้แปรผัน</th><th colspan="4" class="group-deduct">รายการหัก</th><th colspan="4" class="group-result">ผลลัพธ์</th><th class="sticky-action" rowspan="2">จัดการ</th></tr><tr><th>แผนก</th><th>ฐานเงินเดือน</th><th>เงินเดือนรอบนี้</th><th>Recurring</th><th>Commission</th><th>Incentive</th><th>Bonus</th><th>อื่น ๆ</th><th>ขาด/สาย</th><th>ปกส.</th><th>ภาษี</th><th>หักอื่น</th><th>Gross</th><th>Net Pay</th><th>Δ เดือนก่อน</th><th>Employer Cost</th></tr></thead><tbody>${items.map(item=>{const a=adjMap.get(Number(item.employee_id))||{commission:0,incentive:0,bonus:0,otherEarn:0,otherDeduct:0};const bd=payrollBreakdown(item),rec=(bd.recurring_components||[]),recEarn=rec.filter(x=>x.type!=='deduction').reduce((x,c)=>x+Number(c.amount||0),0);const search=`${item.nickname||''} ${item.first_name||''} ${item.last_name||''} ${item.employee_code||''} ${item.department_name||''}`.toLowerCase();return `<tr data-payroll-search="${escapeAttr(search)}"><td class="sticky-col employee-col"><button class="payroll-person payroll-person-button" type="button" onclick="window.openPayrollEmployeeDetail(${Number(item.employee_id)})"><span class="payroll-person-avatar">${escapeHtml((item.nickname||item.first_name||'?').slice(0,1))}</span><span><strong>${escapeHtml(item.nickname||item.first_name)} ${escapeHtml(item.last_name||'')}</strong><small>${escapeHtml(item.employee_code||'')} · ${escapeHtml(item.position_name||'—')}</small></span></button></td><td>${escapeHtml(item.department_name||'—')}</td><td>${money(item.base_salary)}</td><td>${money(item.prorated_salary)}${Number(item.base_salary)!==Number(item.prorated_salary)?'<small>Prorate</small>':''}</td><td>${recEarn?money(recEarn):'—'}${rec.length?`<small>${rec.filter(x=>x.type!=='deduction').map(x=>escapeHtml(x.name)).join(' · ')}</small>`:''}</td><td class="editable-pay-cell">${payrollGridInput(p.id,item.employee_id,'commission',a.commissionGrid,editable)}${Number(a.commission||0)?`<small>รวม ${money(a.commission)}</small>`:''}</td><td class="editable-pay-cell">${payrollGridInput(p.id,item.employee_id,'incentive',a.incentiveGrid,editable)}${Number(a.incentive||0)?`<small>รวม ${money(a.incentive)}</small>`:''}</td><td class="editable-pay-cell">${payrollGridInput(p.id,item.employee_id,'bonus',a.bonusGrid,editable)}${Number(a.bonus||0)?`<small>รวม ${money(a.bonus)}</small>`:''}</td><td class="editable-pay-cell">${payrollGridInput(p.id,item.employee_id,'other',a.otherGrid,editable)}${Number(a.otherEarn||0)?`<small>รวมปรับ ${money(a.otherEarn)}</small>`:''}</td><td>${Number(item.attendance_deduction||0)?money(item.attendance_deduction):'—'}<small>${Number(item.absent_days||0)} วัน · ${Number(item.late_minutes||0)} นาที</small></td><td>${money(item.social_security)}</td><td>${money(item.withholding_tax)}</td><td class="editable-pay-cell">${payrollGridInput(p.id,item.employee_id,'other_deduction',a.otherDeductGrid,editable)}${Number(a.otherDeduct||0)?`<small>รวม ${money(a.otherDeduct)}</small>`:''}</td><td class="gross-col"><strong>${money(item.gross_income)}</strong></td><td class="net-col"><strong>${money(item.net_pay)}</strong></td><td>${payrollVarianceBadge(item)}<small>${Number(item.prior_net_pay||0)?money(item.prior_net_pay):'ไม่มีเดือนก่อน'}</small></td><td><strong>${money(item.employer_cost||item.gross_income)}</strong><small>บริษัท ปกส. ${money(item.employer_social_security||0)}</small></td><td class="sticky-action">${editable?`<button class="payroll-adjust-btn" onclick="window.openPayrollAdjustment(${p.id},${item.employee_id})">ปรับ</button>`:''}<button class="text-btn" onclick="window.openPayrollEmployeeDetail(${item.employee_id})">ดู</button></td></tr>`}).join('')}</tbody><tfoot><tr><td class="sticky-col employee-col"><strong>รวมทั้งรอบ</strong></td><td colspan="11"></td><td class="gross-col"><strong>${money(p.gross_total)}</strong></td><td class="net-col"><strong>${money(p.net_total)}</strong></td><td></td><td><strong>${money(employerCost)}</strong></td><td class="sticky-action"></td></tr></tfoot></table></div>
