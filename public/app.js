...
let documentTemplateSeedAttempted=false;
async function refreshDocuments(){try{let [docs,sys]=await Promise.all([api('/api/documents'),api('/api/document-system/overview')]);if(!(sys.templates||[]).length&&!documentTemplateSeedAttempted&&['owner','co_owner','hr_admin','hr'].includes(String(activeCompanyRole()||''))){documentTemplateSeedAttempted=true;try{await api('/api/document-templates/seed',{method:'POST',body:'{}'});sys=await api('/api/document-system/overview');}catch(seedError){console.warn('standard document templates unavailable',seedError)}}state.documents=docs;state.documentSystem=sys;renderDocuments();}catch(e){console.warn(e)}}
const documentTypeMeta={EMP_CERT:['รับรองการทำงาน','ข้อมูลการจ้างงาน','DOC'],SAL_CERT:['รับรองเงินเดือน','ข้อมูลตำแหน่งและเงินเดือน','฿'],PROB_PASS:['ผ่านทดลองงาน','ต้อง HR ตรวจและพนักงานรับทราบ','✓'],SAL_ADJ:['ปรับเงินเดือน','ต้อง HR ตรวจและพนักงานรับทราบ','↗'],POLICY_ACK:['เอกสารให้รับทราบ','พนักงานอ่าน รับทราบ และชี้แจงได้','ACK'],WARNING:['หนังสือเตือน','ควรออกจาก HR Case เพื่อมีหลักฐานครบ','!']};
function renderDocumentTypeCards(){const templates=state.documentSystem?.templates||[],root=$('#documentTypeCards');if(!root)return;root.innerHTML=templates.map(t=>{const meta=documentTypeMeta[t.code]||[t.name,t.description||'เอกสารพนักงาน','DOC'];return `<button type="button" class="document-type-card" data-template-id="${Number(t.id)}" onclick="selectDocumentTemplate(${Number(t.id)})"><span class="doc-symbol">${escapeHtml(meta[2])}</span><span><strong>${escapeHtml(t.name)}</strong><small>${escapeHtml(meta[1])}</small></span></button>`}).join('')||'<div class="document-draft-preview"><strong>กำลังเตรียม Template มาตรฐาน</strong><small>กดรีเฟรชอีกครั้งหากรายการยังไม่แสดง</small></div>';}
window.selectDocumentTemplate=id=>{const t=(state.documentSystem?.templates||[]).find(x=>Number(x.id)===Number(id));if(!t)return;$('#documentTemplate').value=String(id);document.querySelectorAll('.document-type-card').forEach(x=>x.classList.toggle('active',Number(x.dataset.templateId)===Number(id)));const e=state.employees.find(x=>Number(x.id)===Number($('#documentEmployee').value));const employeeName=e?(e.nickname||`${e.first_name||''} ${e.last_name||''}`.trim()):'พนักงาน';const warning=t.code==='WARNING'?'<br><b>แนะนำ:</b> ใบเตือนควรเริ่มจาก HR Case เพื่อผูกเหตุการณ์ หลักฐาน และคำชี้แจง':'';$('#documentDraftPreview').innerHTML=`<p class="kicker">DOCUMENT PREVIEW</p><strong>${escapeHtml(t.name)} · ${escapeHtml(employeeName)}</strong><small>${t.approval_required?'สร้างเป็น Draft → HR ตรวจ → อนุมัติ → PDF Final':'ระบบสามารถสร้างเอกสารได้ทันที'}${t.acknowledgement_required?' → ส่งให้พนักงานรับทราบ/ชี้แจง':''}${warning}</small>`;};
function openDocumentGenerateModal(){const employees=state.employees.filter(e=>e.status==='active');$('#documentEmployee').innerHTML=employees.map(e=>`<option value="${e.id}">${escapeHtml(e.nickname||e.first_name)} · ${escapeHtml(e.employee_code)}</option>`).join('');$('#documentTemplate').value='';$('#documentNote').value='';$('#documentDraftPreview').innerHTML='<p class="kicker">DOCUMENT PREVIEW</p><strong>เลือกประเภทเอกสารเพื่อดู Workflow</strong><small>ข้อมูลจริงจะถูกดึงจากแฟ้มพนักงานเมื่อสร้าง Draft</small>';renderDocumentTypeCards();$('#documentEmployee').onchange=()=>{const id=Number($('#documentTemplate').value);if(id)selectDocumentTemplate(id)};$('#documentGenerateModal').showModal();}
async function generateEmployeeDocument(){const button=$('#documentGenerateSaveBtn');button.disabled=true;button.textContent='กำลังสร้าง Draft…';try{const templateId=Number($('#documentTemplate').value);if(!templateId)throw new Error('กรุณาเลือกประเภทเอกสาร');const result=await api('/api/document-workflows/create',{method:'POST',body:JSON.stringify({employee_id:Number($('#documentEmployee').value),template_id:templateId,note:$('#documentNote').value.trim()})});$('#documentGenerateModal').close();await refreshDocuments();toast(`สร้าง Draft ${result.document_number} แล้ว · กรุณาตรวจและอนุมัติก่อนส่ง`);}catch(e){toast(e.message,true)}finally{button.disabled=false;button.textContent='สร้าง Draft เพื่อตรวจสอบ';}}
